...
async def test_get_user_by_id_by_admin_success(
    ...
    # 1. إنشاء مستخدم هدف
    email = "mongo_get_by_id_admin@test.com"
...
async def test_get_user_by_id_by_superadmin_success(
    ...
    # 1. إنشاء مستخدم هدف
    email = "mongo_get_by_id_superadmin@test.com"
    user = await create_user_usecase.execute(
        email=email,
...
async def test_get_user_by_id_user_forbidden(
    ...
    # 1. إنشاء مستخدم هدف
    email = "mongo_get_by_id_forbidden@test.com"
    user = await create_user_usecase.execute(
        email=email,
...
async def test_get_user_by_id_not_found(
    ...
    with pytest.raises(UserDoesNotExists) as exc_info:
        await get_user_by_id_usecase.execute(
            user_id="non-existent-mongo-user-id-9999",
            token=admin_token,
        )
...
async def test_get_user_by_id_without_token(
    ...
    email = "mongo_get_by_id_no_token@test.com"
    user = await create_user_usecase.execute(
        email=email,
        name="User No Token",
        password="password123",
...
async def test_get_user_by_id_with_invalid_token(
    ...
    email = "mongo_get_by_id_invalid_token@test.com"
    user = await create_user_usecase.execute(
        email=email,
