...
@pytest.fixture
def user1_token():
    return generate_token(JWTPayload(id="pg-user-1", email="user1@test.com", role=Role.USER.value))


@pytest.fixture
def user2_token():
    return generate_token(JWTPayload(id="pg-user-2", email="user2@test.com", role=Role.USER.value))


@pytest.fixture
def admin1_token():
    return generate_token(JWTPayload(id="pg-admin-1", email="admin1@test.com", role=Role.ADMIN.value))


@pytest.fixture
def admin2_token():
    return generate_token(JWTPayload(id="pg-admin-2", email="admin2@test.com", role=Role.ADMIN.value))


@pytest.fixture
def superadmin_token():
    return generate_token(JWTPayload(id="pg-super-1", email="super@test.com", role=Role.SUPERADMIN.value))


@pytest.mark.integration
async def test_create_post_usecase_postgres(post_repository: PostRepositoryPostgresSQL, user1_token: str):
    create_uc = CreatePostUseCase(post_repository)
    post = await create_uc.execute(
        title="UC PG Title",
        content="UC PG Content",
        token=user1_token,
    )

    assert post.id is not None
    assert post.title == "UC PG Title"
    assert post.author_id == "pg-user-1"
    assert post.author_role == Role.USER.value


@pytest.mark.integration
async def test_get_and_list_post_usecase_postgres(post_repository: PostRepositoryPostgresSQL, user1_token: str):
    create_uc = CreatePostUseCase(post_repository)
...
# ----------------------------------------------------------------------
# سيناريوهات الصلاحيات والتعديل (UpdatePostUseCase RBAC)
# ----------------------------------------------------------------------

@pytest.mark.integration
async def test_update_post_by_author_success_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str
):
...
@pytest.mark.integration
async def test_user_cannot_update_other_user_post_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str, user2_token: str
):
...
@pytest.mark.integration
async def test_admin_can_update_regular_user_post_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str, admin1_token: str
):
...
@pytest.mark.integration
async def test_admin_cannot_update_another_admin_post_postgres(
    post_repository: PostRepositoryPostgresSQL, admin1_token: str, admin2_token: str
):
...
@pytest.mark.integration
async def test_admin_cannot_update_superadmin_post_postgres(
    post_repository: PostRepositoryPostgresSQL, superadmin_token: str, admin1_token: str
):
...
@pytest.mark.integration
async def test_superadmin_can_update_any_post_postgres(
    post_repository: PostRepositoryPostgresSQL, admin1_token: str, superadmin_token: str
):
...
# ----------------------------------------------------------------------
# سيناريوهات الصلاحيات والحذف (DeletePostUseCase RBAC)
# ----------------------------------------------------------------------

@pytest.mark.integration
async def test_delete_post_by_author_success_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str
):
...
@pytest.mark.integration
async def test_user_cannot_delete_other_user_post_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str, user2_token: str
):
...
@pytest.mark.integration
async def test_admin_can_delete_regular_user_post_postgres(
    post_repository: PostRepositoryPostgresSQL, user1_token: str, admin1_token: str
):
...
@pytest.mark.integration
async def test_admin_cannot_delete_another_admin_post_postgres(
    post_repository: PostRepositoryPostgresSQL, admin1_token: str, admin2_token: str
):
...
@pytest.mark.integration
async def test_admin_cannot_delete_superadmin_post_postgres(
    post_repository: PostRepositoryPostgresSQL, superadmin_token: str, admin1_token: str
):
...
@pytest.mark.integration
async def test_superadmin_can_delete_any_post_postgres(
    post_repository: PostRepositoryPostgresSQL, admin1_token: str, superadmin_token: str
):
...
# ----------------------------------------------------------------------
# سيناريوهات التحقق من وجود المستخدم والحذف المتتابع للبوستات (Postgres)
# ----------------------------------------------------------------------

@pytest.mark.integration
async def test_post_operations_reject_nonexistent_or_deleted_user_postgres(
    post_repository: PostRepositoryPostgresSQL,
    auth_repository: AuthRepositoryPostgresSQl,
):
    """التحقق من أن أي عملية بوست ترفض إذا كان المستخدم غير موجود أو محذوفاً في قاعدة البيانات"""
    ...
    ghost_token = generate_token(JWTPayload(id="ghost-pg-user", email="ghost@test.com", role=Role.USER.value))
    ...
    real_user = await auth_repository.create_user("real_author_pg@test.com", "Real Author", "pwd123")
...
@pytest.mark.integration
async def test_delete_user_cascades_posts_postgres(
    post_repository: PostRepositoryPostgresSQL,
    auth_repository: AuthRepositoryPostgresSQl,
):
    """عند حذف مستخدم مهما كانت رتبته، تحذف جميع البوستات الخاصة به تلقائياً"""
    create_uc = CreatePostUseCase(post_repository=post_repository, auth_repository=auth_repository)
    ...
    admin_user = await auth_repository.create_user(
        "admin_exec_pg@test.com", "Admin Exec", "pwd123", role=Role.ADMIN
    )
    ...
    target_user = await auth_repository.create_user(
        "author_to_delete_pg@test.com", "Author To Delete", "pwd123", role=Role.USER
    )
    user_token = generate_token(JWTPayload(id=target_user.id, email=target_user.email, role=Role.USER.value))

    post1 = await create_uc.execute("Post 1 Title", "Content 1", token=user_token)
    post2 = await create_uc.execute("Post 2 Title", "Content 2", token=user_token)

    # التأكد من وجود البوستين
