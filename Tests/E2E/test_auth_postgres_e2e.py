...
def pg_client():
    """
    E2E Client مع PostgreSQL Session بنظام الـ Savepoint،
    يتم عمل Rollback تلقائي بعد كل سيناريو لحذف كافة التغييرات من قاعدة البيانات.
    """
...
def admin_token():
    return generate_token(
        JWTPayload(id="pg-e2e-admin", email="admin_pg_e2e@test.com", role=Role.ADMIN.value)
    )
...
def superadmin_token():
    return generate_token(
        JWTPayload(id="pg-e2e-superadmin", email="super_pg_e2e@test.com", role=Role.SUPERADMIN.value)
    )


# =====================================================================
# 1. دورة حياة المستخدم الكاملة (Complete User Lifecycle Journey)
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_complete_user_lifecycle(pg_client: TestClient, admin_token: str):
    email = "pg_e2e_lifecycle@example.com"
    name = "PG E2E User"
    password = "password123"

    # الخطوة 1: تسجيل مستخدم جديد
    reg_res = pg_client.post(
        "/auth/register",
    ...
    # الخطوة 3: تسجيل الدخول للمستخدم
    ...
    get_id_res = pg_client.get(f"/auth/user/id/{user_id}", headers=admin_headers)
    ...
    post_del_get = pg_client.get(f"/auth/user/id/{user_id}", headers=admin_headers)
    assert post_del_get.status_code == 400


# =====================================================================
# 2. عزل بيانات المستخدمين (Multiple Users Isolation)
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_multiple_users_isolation(pg_client: TestClient, admin_token: str):
    user1_email = "pg_e2e_user1@example.com"
    user2_email = "pg_e2e_user2@example.com"
    pwd = "password123"

    res1 = pg_client.post("/auth/register", json={"name": "User 1", "email": user1_email, "password": pwd})
    res2 = pg_client.post("/auth/register", json={"name": "User 2", "email": user2_email, "password": pwd})
    ...
    admin_headers = {"Authorization": f"Bearer {admin_token}"}
    p1 = pg_client.get(f"/auth/user/id/{u1_id}", headers=admin_headers).json()
    p2 = pg_client.get(f"/auth/user/id/{u2_id}", headers=admin_headers).json()
...
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_admin_cannot_delete_another_admin(
    pg_client: TestClient, admin_token: str, superadmin_token: str
):
    # إنشاء أدمن بواسطة السوبر أدمن
    reg_res = pg_client.post(
        "/auth/register",
        json={
            "name": "Target Admin PG",
            "email": "pg_e2e_target_admin@test.com",
            "password": "password123",
            "role": "admin",
        },
...
# =====================================================================
# 4. فحص جدران الحماية والصلاحيات (RBAC & Auth Guards)
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_rbac_and_auth_guards(pg_client: TestClient):
    reg_res = pg_client.post(
        "/auth/register",
        json={"name": "Guard User", "email": "pg_guard_e2e@test.com", "password": "password123"},
    )
    ...
    assert pg_client.get(f"/auth/user/id/{user_id}", headers=user_headers).status_code == 403
...
# =====================================================================
# 5. أخطاء التسجيل وتسجيل الدخول (Business Constraints)
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_duplicate_email_and_login_failures(pg_client: TestClient):
    email = "pg_dup_login_e2e@test.com"
    pwd = "password123"

    # التسجيل أول مرة
    assert pg_client.post("/auth/register", json={"name": "User", "email": email, "password": pwd}).status_code == 201

    # تكرار التسجيل يرجع 400
    assert pg_client.post("/auth/register", json={"name": "User", "email": email, "password": pwd}).status_code == 400

    # تسجيل دخول بكلمة مرور خاطئة (401)
    assert pg_client.post("/auth/login", json={"email": email, "password": "wrongpassword"}).status_code == 401

    # تسجيل دخول لإيميل غير مسجل (401)
    assert pg_client.post("/auth/login", json={"email": "unreg@test.com", "password": pwd}).status_code == 401


# =====================================================================
# 6. التحقق من صحة المدخلات (Schema Validation 422)
# =====================================================================
@pytest.mark.e2e
def test_pg_e2e_input_validation_errors(pg_client: TestClient):
    # بريد غير صالح
